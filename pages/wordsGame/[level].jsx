...
const wordsGamePage = ({ data, userData }) => {
  ...
  return (
    <div className="bg-[var(--color2)] pt-20 h-screen">
      <BackHeader largeTitle={`Game of Words: ${level}`} parentTitle={'Volver'} />
      {!currentItem && <LoadingScreen />}
      <div className="xl:flex justify-center xl:px-12">
        ...
        <div className="px-4 xl:w-1/2 xl:px-0">
          ...
          <p className="xl:text-base text-sm text-gray-900">{currentItem?.description}</p>
        </div>
      </div>
      <div className="flex px-8 justify-between w-full right-0 xl:px-20 overflow-hidden bg-[var(--color2Shadow)] absolute pb-20 sm:pb-24 md:pb-4 xl:pb-6 pt-2 sm:pt-4 xl:pt-6 rounded-t-xl bottom-0">
        <button
          onClick={currentIndex === 0 ? noPrev : handlePrev}
          className={`group w-[20%] sm:w-[45%] xl:w-[30%] xl:py-7 sm:py-3 py-2 px-1 flex gap-4 items-center justify-center rounded-xl bg-white hover:shadow-md hover:shadow-gray-700 hover:-translate-y-1 ${
            currentIndex == 0 && 'disabled grayscale hover:translate-y-0'
          }`}
        >
          <p className="text-[var(--color3)] xl:text-3xl hidden sm:block">ANTERIOR</p>
          <BsFillArrowLeftCircleFill className="text-3xl fill-[var(--color3)] group-hover:-translate-x-1" />
        </button>
        <button
          onClick={currentIndex === filteredData.length - 1 ? handleOpen : handleNext}
          className="group w-[20%] sm:w-[45%] xl:w-[30%] xl:py-7 sm:py-3 py-2 px-1 flex gap-4 items-center justify-center rounded-xl bg-[var(--color3)] hover:shadow-md hover:shadow-gray-700 hover:-translate-y-1"
        >
          <BsFillArrowRightCircleFill className="group-hover:translate-x-1 text-3xl fill-white" />
          <p className="text-white xl:text-3xl hidden sm:block">SIGUIENTE</p>
        </button>
      </div>
      <Modal
      ...
      >
        <Fade in={open}>
          <Box sx={style}>
            ...
            <p className="text-2xl my-2 text-[var(--color2)]">
              Has Completado Exitosamente: <small className="text-[var(--color3)] text-4xl">{level}</small>
            </p>
            <Lottie className="h-64 w-64 mx-auto" animationData={astronauta} />
            ...
            <div className="flex justify-around gap-4 my-4">
              <button
                ...
                type="submit"
                className="bg-[var(--color3)] group hover:shadow-md hover:shadow-gray-700 hover:scale-105 flex items-center gap-3 justify-center  py-2 px-5 text-lg text-white rounded-md"
              >
                <p className="text-orange-100">Repetir</p>
                <TfiReload className="bg-orange-100 group-hover:rotate-180 text-4xl p-2 rounded-full fill-orange-600" />
              </button>
              <button
                onClick={completeModule}
                type="submit"
                className="bg-green-600 hover:shadow-md hover:shadow-gray-700 hover:scale-105 flex items-center gap-3 justify-center  py-2 px-5 text-lg text-white rounded-md"
              >
                <p className="text-green-200">Completado</p>
