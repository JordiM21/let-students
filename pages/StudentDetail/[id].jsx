...
export default function () {
  ...
  const [dateVariable, setDateVariable] = useState(user?.dateVariable)
  ...
  const [payment, setPayment] = useState({
    ...
    frequency: user?.payment?.frequency || '',
    method: user?.payment?.method || '',
    platform: user?.payment?.platform || '',
  })
  ...
  useEffect(() => {
    ...
    if (user?.startDate) {
      setCurrentMonth(getCurrentMonthWithAdjustment(user.startDate, user.dateVariable ?? 0))
      setStartDate(user.startDate)
    }
  }, [user])

  const updateDateVariable = async (e) => {
    e.preventDefault()
    const nameRef = doc(db, 'users', user.id)
    await updateDoc(nameRef, {
      dateVariable,
    })
    toast.success('succesfully changed the variable!')
    setTimeout(() => {
      router.reload()
    }, 1000)
  }
  ...
  return (
    <div className="mx-4 max-w-xl pt-20 max-md:pb-32 md:pb-4 md:mx-auto">
      <BackHeader largeTitle={'Modify Student Data'} parentTitle="Volver" />
      <div className="flex justify-center">
        <YourProfile char={user?.profileImg} />
      </div>
      <h2>
        Currently Modifying{' '}
        <span className="text-[var(--color3)] font-semibold">
      ...
      <ul className="space-y-3">
        {user?.schedule?.map((item) => (
          <li
            key={item.day}
            className="w-11/12 mx-auto py-3 px-8 rounded-lg bg-gray-400 flex justify-between items-center"
          >
            <span>{item.day}</span>
            <button className="bg-red-600 p-2 rounded-lg" onClick={() => deleteScheduleItem(item.day)}>
              {' '}
              <BsTrashFill fill="white" />{' '}
            </button>
          </li>
        ))}
      </ul>
      <form onSubmit={changeSchedule} className="my-8 space-y-4">
        {user?.schedule?.length > 0 && <Schedule schedule={user.schedule} />}
        ...
        <button type="submit" className="w-full py-4 bg-[var(--color2)] text-white font-bold hover:opacity-75">
          Add day to schedule
        </button>
      </form>
      ...
      <form onSubmit={updateDateVariable} className="my-8">
        <FormControl variant="filled" className="w-full">
          <TextField
            className="w-full"
            variant="filled"
            label="Modify the Date Variable"
            placeholder='ex: -60 for vacations and +30 for early progress'
            value={dateVariable}
            onChange={(e) => setDateVariable(e.target.value)}
          />
          <FormHelperText>Start Date: {startDate}</FormHelperText>
          <FormHelperText>current Date Variation: {user?.dateVariable}</FormHelperText>
          <FormHelperText>current Month: {currentMonth}</FormHelperText>
        </FormControl>
        <button type="submit" className="w-full py-4 bg-[var(--color2)] text-white font-bold hover:opacity-75">
      ...
      <form onSubmit={updatePayment} className="my-8 space-y-6">
        {/* Payment Date */}
        <FormControl variant="filled" className="w-full">
          <TextField
            className="w-full"
          ...
          />
          <FormHelperText>Current Payment Date: {user?.payment?.date}</FormHelperText>
        </FormControl>
        ...
        <FormControl variant="filled" className="w-full">
          ...
          <Select
            ...
            onChange={(e) => setPayment((prev) => ({ ...prev, frequency: e.target.value }))}
          >
            <MenuItem value="">
              <em>None</em>
            </MenuItem>
          ...
          <FormHelperText>Current Frequency: {user?.payment?.frequency}</FormHelperText>
        </FormControl>
        ...
        <FormControl variant="filled" className="w-full">
          ...
          <FormHelperText>Current Method: {user?.payment?.method}</FormHelperText>
        </FormControl>

        {/* Payment Platform */}
        <FormControl variant="filled" className="w-full">
          <InputLabel id="payment-platform-label">Payment Platform</InputLabel>
          <Select
            labelId="payment-platform-label"
            value={payment.platform}
            onChange={(e) => setPayment((prev) => ({ ...prev, platform: e.target.value }))}
          >
            <MenuItem value="">
              <em>None</em>
            </MenuItem>
          ...
          <FormHelperText>Current Platform: {user?.payment?.platform}</FormHelperText>
        </FormControl>
      ...
      <form onSubmit={updateRole} className="bg-yellow-400 p-4 rounded-md">
        ...
        <button type="submit" className="w-full py-4 bg-[var(--color4)] text-white font-bold hover:opacity-75">
          Change Role
        </button>
      </form>
    </div>
